...
def read_result(result_file):
    ...
    with open(result_file, "r") as f:
        for line in f.readlines():
            tmp = line.strip().split(' ')
            preds += [(float(tmp[0]), float(tmp[1]), float(tmp[2]), float(tmp[3]), float(tmp[4]))]
...
if __name__ == "__main__":
    ...
    for filename in list_files:
        ...
        list_outputs = detector.detect(img) # read_result(os.path.join(result_folder, filename[:-4] + ".txt"))
        preds = get_predict(list_outputs)
        print(preds[0])

        metric_fn = MetricBuilder.build_evaluation_metric("map_2d", async_mode=True, num_classes=1)
        metric_fn.add(preds, targets)
        score = metric_fn.value(iou_thresholds=np.arange(0.25, 0.75, 0.05))['mAP']
